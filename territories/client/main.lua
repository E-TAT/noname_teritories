...
ShowNotification = function(msg)
  exports['okokNotify']:Alert("Territories", msg, 5000, 'info')
...
Start = function()
  ...
  for k,v in pairs(Territories) do
    ...
    for _,area in pairs(Territories[k].areas) do
      local blipHandle = mLibs:AddRadiusBlip(area.location.x,area.location.y,area.location.z,area.height,area.width,area.heading,BlipColors[v.control],math.floor(v.influence),true,area.display)
        ...
        if sellItem then
          ...
          if progBar then
            ...
            exports.rprogress:Custom({
                    Duration = (Config.SellDrugsTimer),
                    Label = (_U["selling_drugs"]),
                    DisableControls = {
                        Mouse = true,
                        Player = true,
                        Vehicle = true
      ...
      for handle,blip in pairs(v.blips) do
        if Config.DisplayZoneForAll or PlayerInGang() then
          if blip.color ~= BlipColors[v.control] or blip.alpha ~= math.floor(v.influence) then 
            ...
            mLibs:SetBlip(handle,"display",3)
...
GetClosestZone = function()
  ...
  for k,v in pairs(Territories) do
   DrawMarker(1, v.loc, 0, 0, 0, 0, 0, 0, 298.0, 298.0, 50.0, MarkerColors[v.control].r, MarkerColors[v.control].g, MarkerColors[v.control].b, 155, 0, 0, 2, 0, 0, 0, 0) 
    if GetDistanceBetweenCoords(coords, v.loc, true) < 151 then
      closest = k
    end
  end
